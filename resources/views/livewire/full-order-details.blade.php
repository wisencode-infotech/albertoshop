<div class="flex w-full flex-col border border-border-200 bg-white" style="overflow: auto;">
    ...
    <div class="flex flex-col border-b border-border-200 sm:flex-row">
        ...
        <div class="flex w-full flex-col px-5 py-4 md:w-2/5">
            ...
            <div class="mb-3 flex justify-between"><span class="text-sm text-body">Discount @if($order_data->coupon_code)<br><span class="px-1 rounded-full text-light text-xs bg-accent ">{{ $order_data->coupon_code }}</span>@endif</span><span class="text-sm text-heading">{{ $order_data->currency->symbol }} {{ number_format($order_data->discount_amount, 2) }}</span></div>
